...
/* `clip` (unlike `hidden`) contains fixed/transformed descendants without
   forming a scroll container, so a floating element can never add a stray
   horizontal scrollbar, while the sticky section rail keeps working. */
...
.cal-evidence-row {
  grid-template-columns: minmax(0, 180px) auto minmax(0, 1fr) !important;
  min-width: 0;
}
.cal-evidence-row > span {
  min-width: 0;
  overflow-wrap: anywhere;
}
.cal-evidence-row > span:nth-child(2) {
  white-space: nowrap;
...
def _favicon_link() -> str:
    """Return a `<link rel="icon">` with the Caliper mark as an SVG data URI.

    Browser tabs were untitled-with-globe before this — the embedded SVG is
    tiny (~200 bytes) and resolves to the same caliper glyph the masthead
    uses. CSS variables don't resolve in favicons, so the stroke colour is a
    literal hex that matches `--accent` in the default dark theme.
    """
    svg = (
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">'
        '<path d="M3 7v10M21 7v10M3 7l4 -3M21 7l-4 -3M3 17l4 3M21 17l-4 3M7 12h10" '
        'fill="none" stroke="#7cc4ff" stroke-width="1.75" '
        'stroke-linecap="round" stroke-linejoin="round" />'
        '<circle cx="12" cy="12" r="1.75" fill="#7cc4ff" />'
        "</svg>"
    )
    payload = base64.b64encode(svg.encode("utf-8")).decode("ascii")
    return f'<link rel="icon" type="image/svg+xml" href="data:image/svg+xml;base64,{payload}">'
...
def _section_overview(d: Dashboard, *, dense: bool, rhythm: str) -> str:
    ...
    body = (
        ...
        'style="display:grid;gap:12px;align-items:stretch">'
        + cards
        + "</div>"
...
def _section_budgets(d: Dashboard, *, rhythm: str) -> str:
    ...
    for b in d.budgets:
        ...
        if b.spent >= b.cap:
            ...
            tone_color = "var(--ok)"
        rows_html.append(
            '<li class="cal-budget-row">'
            f'<span class="cal-budget-period" style="color:var(--ink);font-weight:500">'
            f"{_esc(b.period)}</span>"
            f'<span class="cal-budget-bar-cell">{_budget_bar(b.spent, b.cap, b.warn)}</span>'
            f'<span class="cal-budget-pct" '
            f'style="font-family:var(--mono);color:{tone_color};font-size:12px">{pct:.0f}%</span>'
            '<span class="cal-budget-amount" '
            'style="font-family:var(--mono);color:var(--ink-2);font-size:12px">'
