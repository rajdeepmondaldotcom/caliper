...
def _usage_delta(current: Usage, previous: Usage | None) -> Usage:
    ...
    if (
        ...
        or current.cache_creation_input_tokens < previous.cache_creation_input_tokens
        or current.cache_read_input_tokens < previous.cache_read_input_tokens
        or current.cache_creation_input_1h_tokens < previous.cache_creation_input_1h_tokens
    ...
    return Usage(
        ...
        cache_creation_input_tokens=max(
            0, current.cache_creation_input_tokens - previous.cache_creation_input_tokens
        ),
        cache_read_input_tokens=max(
            0, current.cache_read_input_tokens - previous.cache_read_input_tokens
        ),
        cache_creation_input_1h_tokens=max(
            0, current.cache_creation_input_1h_tokens - previous.cache_creation_input_1h_tokens
        ),
    ...
    if isinstance(last_raw, dict):
        ...
        if total_usage is not None:
            ...
            new_total = Usage(
                ...
                cache_creation_input_tokens=previous_total.cache_creation_input_tokens
                + usage.cache_creation_input_tokens,
                cache_read_input_tokens=previous_total.cache_read_input_tokens
                + usage.cache_read_input_tokens,
                cache_creation_input_1h_tokens=previous_total.cache_creation_input_1h_tokens
                + usage.cache_creation_input_1h_tokens,
...
def current_config_service_tier(config_path: Path) -> str:
    try:
        ...
        return ""
    tier = normalize_tier(raw.get("service_tier"))
    if tier:
        return tier
...
def _override_tier(item: dict) -> str:
    tier = normalize_tier(str(item.get("service_tier") or item.get("tier") or ""))
...
def _tier_from_payload(payload: dict, fallback: str) -> str:
    return normalize_tier(payload.get("service_tier")) or fallback
...
def model_for_event(
    ...
    current_model = normalize_model(VENDOR_OPENAI_CODEX, current_meta.model)
    if current_model and current_model_source:
        return current_model, current_model_source, False
    state_model = normalize_model(VENDOR_OPENAI_CODEX, thread_meta.model)
    if state_model:
        return state_model, "state-db", False
    fallback = normalize_model(VENDOR_OPENAI_CODEX, default_model)
        ...
        for path in paths:
            ...
            if cache:
                ...
                if get_records is not None:
                    parsed = get_records(path, signature, start=start_utc, end=end_utc)
                if parsed is None:
...
def _event_matches_project(event: UsageEvent, project: str) -> bool:
    target = project.strip()
    if not target:
        return True
    candidates = {
        event.thread.cwd,
        Path(event.thread.cwd).name if event.thread.cwd else "",
...
def _usage_event_from_token_count(
    *,
    ...
    return UsageEvent(
        ...
        usage_source=usage_source,
        model_context_window=model_context_window,
        dedupe_key=_event_dedupe_key(path, event_time, usage_source, usage),
        raw_model=current_meta.model or thread_meta.model or options.default_model,
        **rate_limit_fields(rate_limits),
    )
...
def _event_dedupe_key(
    ...
    return "|".join(
        [
            ...
            str(usage.cache_creation_input_tokens),
            str(usage.cache_read_input_tokens),
