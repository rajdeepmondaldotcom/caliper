...
Vendor product names (`Claude Code`, `Anthropic`, `OpenAI`,
`Cursor`, `Aider`) and model identifiers (`claude-opus-4.7`,
`gpt-5.5`, ...) are not attribution. Leave them alone in copy,
tests, parsers, and pricing tables.
