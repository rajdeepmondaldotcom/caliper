...
New commits use the human author identity only. Do not add assistant/tool
co-author trailers or generated-by marketing lines. Past commit identities
are normalized with `.mailmap` where possible.

Vendor product names (`Claude Code`, `Anthropic`, `OpenAI`) and model
identifiers (`claude-opus-4.7`, `gpt-5.5`, ...) are not attribution. Leave
them alone in copy, tests, parsers, and pricing tables.

`tests/test_attribution_policy.py` enforces this on tracked files.
...
6. **Small runtime dependency list.** Today: `click`, `rich`, `typer`,
   `textual`, and `watchdog`. The optional `[prom]` extra brings
   `prometheus-client`. Adding anything else requires a real reason.
